...
ERROR_CODES = {	304 : ('Not Modified', 'There was no new data to return.'),
...
def raiseResponseError(code):
    error = ERROR_CODES.get(code,('Unknown Error',''))
    raise TwythonError(error[0],code,error[1])
...
class TwythonError(AttributeError):
    ...
    """
    def __init__(self, msg, error_code=None,extended_msg=''):
        self.msg = msg
        self.extended_msg = extended_msg
        if error_code == 400:
            raise APILimit(msg)
...
class Twython(object):
    ...
    def _media_update(self, url, file_, params={}):
        ...
        req = requests.post(url, data=params, files=file_, headers=self.headers,verify=False)
        ...
        if status_code == 200:
            ...
            raiseResponseError(status_code)
